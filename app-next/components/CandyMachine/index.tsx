...
import { fetchCandyMachine, fetchCandyGuard, CandyMachine as CandyMachineType } from "@metaplex-foundation/mpl-candy-machine";
...
// export default async function CandyMachine({ walletAddress }) {
const CandyMachine = (props: CandyMachineProps) => {
  ...
  const getCandyMachineState = async () => {
    ...
    if (process.env.NEXT_PUBLIC_SOLANA_RPC_HOST && process.env.NEXT_PUBLIC_CANDY_MACHINE_ID) {
      // const umi = createUmi(process.env.NEXT_PUBLIC_SOLANA_RPC_HOST).use(mplCandyMachine());
      const umi = createUmi(process.env.NEXT_PUBLIC_SOLANA_RPC_HOST);
      const candyMachineV3 = await fetchCandyMachine(umi, publicKey(process.env.NEXT_PUBLIC_CANDY_MACHINE_ID));
      // TODO: Guardを設定したら、ここで取得する
      // const candyGuard = await fetchCandyGuard(umi, candyMachineV3.mintAuthority);
      console.log(`candyMachineV3.publicKey: ${candyMachineV3.publicKey.toString()}`);
      console.log(`candyMachineV3.items: ${JSON.stringify(candyMachineV3.items)}`);
      console.log(`candyMachine.data.itemsAvailable: ${candyMachineV3.data.itemsAvailable}`);
      console.log(`candyMachineV3.itemsRedeemed: ${candyMachineV3.itemsRedeemed}`);
      // console.log(`candyGuard: ${candyGuard}`);

      setCandyMachine(candyMachineV3);
    }
  ...
  const mintToken = async () => {
    console.log(`Push mint`); // TODO: delete
    //   setIsMinting(true);
    //   // NFTのアカウントを作成する
